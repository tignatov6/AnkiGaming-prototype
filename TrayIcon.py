...
class SysTrayIcon(object):
    ...
    def run(self):
        ...
        Он создает окно и запускает цикл обработки сообщений.
        """
        ...
        # Запуск цикла обработки сообщений Windows. Этот вызов блокирующий.
        win32gui.PumpMessages()

    def stop(self):
        """Отправляет сообщение о закрытии окна из другого потока."""
    ...
    def destroy(self, hwnd, msg, wparam, lparam):
        if self.on_quit:
            self.on_quit(self)  # Вызов колбэка при выходе
        nid = (self.hwnd, 0)
        win32gui.Shell_NotifyIcon(win32gui.NIM_DELETE, nid)
        win32gui.PostQuitMessage(0)  # Завершает цикл PumpMessages
    ...
    def execute_menu_option(self, id):
        ...
        if menu_action == self.QUIT:
            # При выборе 'Quit' мы больше не вызываем destroy напрямую,
            # а просто посылаем сообщение о закрытии.
            # Метод destroy будет вызван автоматически.
